...
rotorCenterLc = 2*rotorTipLc;
farLc = rotorRadius;

wedgeAngle = 15*Pi/180;
...
diskEntities[] = Extrude {{0,1,0},{0,0,0},-wedgeAngle}
{
  Surface{23};
  Layers{1};
  Recombine;
...
Physical Surface("wedge1") = {domainEntities[0], diskEntities[0]};
Physical Surface("inlet") = {domainEntities[5]};
Physical Surface("tunnel") = {domainEntities[6]};
Physical Surface("outlet") = {domainEntities[7]};

Physical Volume("rotatingZone") = {diskEntities[1]};
Physical Volume(1000) = {domainEntities[1], diskEntities[1]};